<template>
  <div
    ...
    class="recommended-articles mt-8 mb-10"
  >
    ...
    <ul class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
      <li
        ...
        :key="article.id"
        class="recommended-article-item bg-white dark:bg-neutral-800 rounded-md shadow-md overflow-hidden"
      >
        <!-- Image de l'article -->
        <img
          v-if="article.image"
        ...
        <div class="p-3">
          <!-- Titre de l'article -->
          <h4
            class="text-base font-semibold text-gray-800 mb-1 dark:text-white"
          >
            {{ article.title }}
          </h4>
          <p class="text-gray-600 text-xs mb-3 dark:text-gray-100">
            {{ stripHTML(article.content).slice(0, 80) }}...
          </p>
...
onMounted(async () => {
  ...
  const response = await fetch(
    `http://localhost:8000/api/article/${articleId}/recommendations`
  );
