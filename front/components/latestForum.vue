<template>
  <section class="home py-8">
    <div class="flex items-center mb-6">
      <img
        src="/medias/icons/chat.svg"
        alt="deux bulles de discussion"
    ...
    <p class="text-gray-600 dark:text-gray-300 mb-8 text-lg leading-relaxed">
      Besoin d'un coup de pouce pour progresser ? Visite notre forum où joueurs
      novices et experts échangent conseils, astuces, et secrets. Poste ta
      question ou aide la communauté à résoudre ses énigmes !
    </p>
    <!-- Conteneur du tableau -->
    <div class="overflow-x-auto shadow-lg rounded-lg">
      <table
        v-if="latestForumPosts"
        class="min-w-full bg-white dark:bg-neutral-800 hidden md:table"
      >
        ...
        <tbody>
          ...
          >
            <td
              class="px-6 py-4 text-gray-700 dark:text-gray-100 border-b dark:border-neutral-700"
            >
              {{ row.title }}
            </td>
            <td
              class="px-6 py-4 text-gray-600 dark:text-gray-300 border-b dark:border-neutral-700"
            >
              {{ row.content.slice(0, 70) + " ..." }}
            </td>
            <td
              class="px-6 py-4 text-gray-600 dark:text-gray-300 border-b dark:border-neutral-700 text-center"
            >
              {{ row.commentCount }}
            </td>
          </tr>
      ...
      <div v-if="latestForumPosts" class="md:hidden space-y-4">
        <div
          v-for="row in forumRows"
          :key="row.id"
          @click="handleRowClick(row)"
          class="bg-white dark:bg-neutral-800 shadow-md rounded-lg p-4 hover:bg-emerald-50 dark:hover:bg-emerald-700 transition cursor-pointer"
        >
...
const { data: latestForumPosts } = await useAsyncData(
  "latest-forum-posts",
  () => {
...
const forumRows = computed(() => {
  return (
    latestForumPosts.value?.map((post) => ({
      id: post.id,
      title: post.title,
