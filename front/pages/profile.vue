<template>
  <div class="h-screen">
    <div class="max-w-5xl mx-auto p-6 bg-gray-100 rounded-lg my-10 shadow-lg">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 flex flex-col items-center md:items-start">
          ...
          <div class="mt-4 text-center md:text-left">
            <h1 class="text-3xl font-semibold text-gray-800">{{ profile.pseudo }}</h1>
        ...
        <div class="md:col-span-2 bg-white rounded-lg shadow-md p-6">
          ...
          <p class="text-gray-700 mt-2">{{ stripHTML(profile.description) || "Aucune description fournie." }}</p>
          <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <p class="text-gray-600"><strong>Âge :</strong> {{ profile.age || 'Non fourni' }}</p>
            <p class="text-gray-600"><strong>Genre :</strong> {{ profile.gender || 'Non fourni' }}</p>
            <p class="text-gray-600"><strong>Discord :</strong> {{ profile.discord || 'Non fourni' }}</p>
          </div>
        </div>
      </div>

      <!-- Section des actions ou statistiques -->
      <div v-if="!profile.error" class="mt-8 bg-white rounded-lg shadow-md p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
