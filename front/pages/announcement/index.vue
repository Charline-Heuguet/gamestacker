<template>
  <div
    class="announcement-container min-h-screen bg-white dark:bg-neutral-900 py-12 px-4"
  >
    ...
    >
      <div
        ...
        :key="announcement.id"
        class="announcement-card relative bg-gray-100 dark:bg-neutral-800 text-gray-800 p-6 rounded-lg shadow"
      >
        <div class="user-image mb-4">
          <img
            :src="
              announcement.user.image
                ? `${backendUrl}/images/users/${announcement.user.image}`
                : `${backendUrl}/images/users/default.jpg`
            "
        ...
        <h2 class="text-xl font-semibold mb-2 mt-8 dark:text-emerald-400">
          {{ announcement.title }}
        </h2>
        <p class="mb-1 dark:text-neutral-300">
          <strong class="text-gray-500 dark:text-neutral-300">Jeu :</strong>
          {{ announcement.game }}
        </p>
        <p class="mb-1 dark:text-neutral-300">
          <strong class="text-gray-500 dark:text-neutral-300">Date :</strong>
          {{ formatDate(announcement.date) }}
        </p>
        <p class="mb-1 dark:text-neutral-300">
          <strong class="text-gray-500 dark:text-neutral-300"
            >Nb max de joueurs :</strong
          >
          {{ announcement.max_nb_players }}
        </p>
        <a
          :href="`/announcement/${announcement.id}`"
          class="mt-4 w-full bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-2 px-4 rounded text-center block"
        >
          Voir l'annonce
        </a>
      </div>
    ...
    >
      ...
      >
        ...
        <form @submit.prevent="createAnnouncement">
          ...
          <div class="mb-4">
            <label for="maxPlayers" class="block text-gray-300"
              >Nombre maximum de joueurs</label
            >
            <input
              v-model.number="newAnnouncement.max_nb_players"
              type="number"
              id="maxPlayers"
              class="w-full p-2 mt-1 rounded bg-gray-700 text-white"
              required
...
const fetchAnnouncements = async (pageNum = 1, term = "") => {
  try {
    const response = await fetch(
      `${backendUrl}/api/announcement?page=${pageNum}&search=${term}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
