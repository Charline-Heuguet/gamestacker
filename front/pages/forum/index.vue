<template>
  ...
  <div
    ...
    :style="{
      backgroundImage: `url('https://i.ibb.co/gSdyxm3/codmobile-hero2.jpg')`,
  ...
  >
    ...
    <p class="text-base md:text-lg text-gray-100 max-w-2xl">
      Posez vos questions, partagez vos expériences et trouvez des partenaires
      de jeu.
    </p>
    ...
    <button
      ...
      class="mt-4 bg-emerald-500 text-white font-semibold py-2 px-4 rounded-full hover:bg-emerald-600"
    >
    ...
    <button
      ...
      class="mt-4 bg-gray-500 text-white font-semibold py-2 px-4 rounded-full hover:bg-gray-600"
    >
  ...
  >
    <div
      class="modal bg-gray-800 text-white p-8 rounded-lg shadow-lg max-w-4xl w-full"
    >
      ...
      <form @submit.prevent="createForum">
        ...
        <div class="flex justify-end space-x-4">
          <button
            type="submit"
            class="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-2 px-4 rounded"
          >
            Créer
          </button>
          <button
  ...
  <div
    class="forum-container bg-white py-12 px-4 rounded-lg mw-80 w-full dark:bg-neutral-900"
  >
    <h2
      class="text-3xl font-bold text-center text-gray-800 mb-8 dark:text-emerald-400"
    >
      Liste des Forums
    </h2>
    ...
    />
    <div v-if="isLoading" class="text-center text-xl text-emerald-500 mt-8">
      Un peu de patience ! Ca arrive
      <UIcon name="svg-spinners:90-ring-with-bg" class="w-6 h-6" />
    </div>
    ...
    <div v-if="forums.length" class="space-y-8">
      <div
        ...
        :key="forum.id"
        class="forum-item bg-gray-100 p-6 rounded-lg shadow-lg dark:bg-neutral-800"
      >
        <a
          v-if="forum.id"
          :href="`/forum/${forum.id}`"
          class="text-xl font-semibold text-emerald-600 underline mb-2"
        >
          {{ forum.title }}
        </a>
        <p v-else class="text-red-500">
          Erreur : ID manquant pour cet article.
        </p>
        <p class="text-gray-700 dark:text-gray-100 mb-2">{{ forum.content }}</p>
        <p class="text-gray-600 dark:text-gray-50">
          <strong>Date :</strong> {{ formatDate(forum.date) }}
        </p>
        <hr class="my-4 border-gray-300 dark:border-gray-700" />
        <p class="text-neutral-600 dark:text-emerald-800">
          <strong> {{ forum.user ? forum.user.pseudo : "Anonyme" }}</strong>
        </p>
      </div>
...
const fetchForums = async (pageNum = 1, term = "") => {
  ...
  try {
    const response = await fetch(
      `http://localhost:8000/api/forum?page=${pageNum}&search=${term}`,
      {
        headers: {
          "Content-Type": "application/json",
        },
